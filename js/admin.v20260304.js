...
function getIdentityApiUrl() {
  ...
  // Netlify Identity is not served by netlify dev unless you configure a live site.
  // To avoid the widget's "Development Settings" prompt, point localhost at the live site.
  if (host === 'localhost' || host === '127.0.0.1' || host === '::1') {
    return 'https://cvprozambia.com/.netlify/identity';
  }

  return origin ? `${origin}/.netlify/identity` : '/.netlify/identity';
}
...
async function checkFunctionsAvailable() {
  try {
    ...
    // On a real Netlify Functions environment this should be 401 (not logged in) or 200 (logged in + admin).
    if (res.status === 404) {
