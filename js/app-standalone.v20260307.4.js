...
async function handleDownload() {
    ...
    try {
        ...
        // Preload BOTH libraries in parallel BEFORE payment (for instant download after payment)
        // Add 15 second timeout to prevent infinite loading
        const loadWithTimeout = Promise.race([
            Promise.all([ensureJsPdfLoaded(), ensureLencoLoaded()]),
            new Promise((_, reject) => setTimeout(() => reject(new Error('Loading timed out. Please refresh and try again.')), 15000))
        ]);
        ...
        try {
            ...
            showToast(loadError?.message || 'Failed to load payment system. Please refresh the page.', 'error');
            if (loadingSpan) loadingSpan.style.display = 'none';
            if (textSpan) textSpan.style.display = 'inline';
            return;
        }
        ...
        window.LencoPay.getPaid({
            ...
            onSuccess: function() {
                ...
                (async () => {
                    try {
                        let pdfForEmail = null;
                        if (product !== 'cover') {
                            // jsPDF already preloaded - generate immediately
                            const pdf = generatePDF(data);
                            pdfForEmail = pdf;
                            pdf.save(`CV_${safeName}.pdf`);
                        }
            ...
            onConfirmationPending: function() {
                ...
                // Generate PDF immediately (jsPDF already preloaded)
                (async () => {
                    try {
                        if (product !== 'cover') {
                            const pdf = generatePDF(data);
