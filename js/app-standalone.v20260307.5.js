...
function startCvBuilder() {
    ...
    setAppMode('cv');
    // Ensure wizard nav buttons are wired up
    initWizard();
...
async function ensureJsPdfLoaded() {
    // Script loads from HTML, just wait for initialization
    if (window.jspdf?.jsPDF) return;
    console.log('[CVPro] Waiting for jsPDF to initialize...');
    // Wait up to 10 seconds for jsPDF (script loads from HTML async)
    for (let i = 0; i < 100 && !window.jspdf?.jsPDF; i++) {
        await new Promise(r => setTimeout(r, 100));
    }
    if (!window.jspdf?.jsPDF) {
        console.error('[CVPro] jsPDF global not found after waiting.');
        throw new Error('PDF generator not ready. Please refresh the page and try again.');
    }
    console.log('[CVPro] jsPDF ready!');
}
...
async function ensureLencoLoaded() {
    ...
    console.log('[CVPro] Waiting for LencoPay to initialize...');
    // Wait up to 10 seconds for LencoPay (script loads from HTML async)
    for (let i = 0; i < 100 && !window.LencoPay?.getPaid; i++) {
        await new Promise(r => setTimeout(r, 100));
    }
    if (!window.LencoPay?.getPaid) {
        console.error('[CVPro] LencoPay not ready. window.LencoPay =', window.LencoPay);
...
function initWizard() {
    // Respect the current app mode (CV vs Cover) so hidden steps aren't included.
    wizardState.steps = computeWizardStepsForMode(getActiveAppMode());
    if (!wizardState.steps.length) return;
    ...
    const nextBtn = document.getElementById('wizardNext');
    if (prevBtn) {
        prevBtn.onclick = () => goToWizardStep(wizardState.currentIndex - 1);
    }
    if (nextBtn) {
        nextBtn.onclick = () => {
...
function addSkill() {
    ...
    const type = typeInput ? typeInput.value : 'technical';
    if (!cvData.skillsGrouped) cvData.skillsGrouped = { technical: [], soft: [], language: [] };
    if (skill && !cvData.skillsGrouped[type].includes(skill)) {
        cvData.skillsGrouped[type].push(skill);
        renderSkills();
        input.value = '';
    }
}
...
function removeSkill(skill) {
    if (!cvData.skillsGrouped) return;
    for (const type of Object.keys(cvData.skillsGrouped)) {
        cvData.skillsGrouped[type] = cvData.skillsGrouped[type].filter(s => s !== skill);
    }
    renderSkills();
}
...
function renderSkills() {
    const types = [
        { key: 'technical', label: 'Technical Skills', containerId: 'skillsListTechnical' },
        { key: 'soft', label: 'Soft Skills', containerId: 'skillsListSoft' },
        { key: 'language', label: 'Language Skills', containerId: 'skillsListLanguage' }
    ];
    if (!cvData.skillsGrouped) cvData.skillsGrouped = { technical: [], soft: [], language: [] };
    types.forEach(({ key, label, containerId }) => {
...
let draggedSkillType = null;
let draggedSkillIndex = null;
window.onSkillDragStart = function(event, type, index) {
    draggedSkillType = type;
    draggedSkillIndex = index;
    event.dataTransfer.effectAllowed = 'move';
};
window.onSkillDragOver = function(event) {
    event.preventDefault();
};
window.onSkillDrop = function(event, type, index) {
    event.preventDefault();
    if (draggedSkillType === null || draggedSkillIndex === null) return;
    if (!cvData.skillsGrouped) return;
    const skillList = cvData.skillsGrouped[draggedSkillType];
    const [movedSkill] = skillList.splice(draggedSkillIndex, 1);
    if (type === draggedSkillType) {
        skillList.splice(index, 0, movedSkill);
    } else {
        cvData.skillsGrouped[type].splice(index, 0, movedSkill);
    }
    draggedSkillType = null;
    ...
    container.innerHTML = cvData.certifications.map((cert, index) => `
        <div class="education-item">
            ...
            <div class="form-group">
                ...
                <input type="text" value="${escapeHtml(cert.name)}" onchange="updateCertification(${cert.id}, 'name', this.value)" required>
            </div>
            <div class="form-group">
                <label>Issuing Organization</label>
                <input type="text" value="${escapeHtml(cert.issuer)}" onchange="updateCertification(${cert.id}, 'issuer', this.value)" required>
            </div>
            <div class="form-group">
                <label>Date Awarded</label>
                <input type="month" value="${escapeHtml(cert.year)}" onchange="updateCertification(${cert.id}, 'year', this.value)" required>
            </div>
...
async function downloadCoverLetterDocx(snapshot, { fileName } = {}) {
    const safeName = String(snapshot?.personalInfo?.fullName || 'Your_Name').trim().replace(/\s+/g, '_');
    const finalName = String(fileName || `Cover_Letter_${safeName}.docx`);

    const res = await fetch('/.netlify/functions/cover-letter-docx', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ snapshot, fileName: finalName })
    });
    ...
    if (!res.ok) {
        ...
        throw new Error(msg || `Cover letter download failed (${res.status})`);
    }
...
// Send CV/Cover Letter via email after download
async function sendCvEmail(data, pdfDoc, product = 'cv') {
    try {
        const email = data?.personalInfo?.email;
        if (!email) {
            console.log('No email address, skipping email send');
            return;
        }
        ...
        let pdfBase64 = '';
        let fileName = '';
        const safeName = String(data?.personalInfo?.fullName || 'CV').trim().replace(/\s+/g, '_');
        
        if (pdfDoc && product !== 'cover') {
            // jsPDF output as base64 (without data:application/pdf;base64, prefix)
            pdfBase64 = pdfDoc.output('datauristring').split(',')[1];
            fileName = `CV_${safeName}.pdf`;
        }

        // For cover letter only, we skip since it's a DOCX (handled server-side)
        if (product === 'cover') {
            // Cover letters are DOCX - we'd need separate handling
            // For now, just skip email for cover-letter-only downloads
            console.log('Cover letter only - email attachment not yet supported for DOCX');
            return;
        }
        ...
        const response = await fetch('/.netlify/functions/send-cv-email', {
            ...
            body: JSON.stringify({
                ...
                pdfBase64,
                fileName,
                customerName: data?.personalInfo?.fullName || '',
                product
            })
        });
        ...
        if (result.success) {
            showToast('CV also sent to your email!', 'success');
        } else {
            console.error('Email send failed:', result.error);
            // Don't show error toast - download already succeeded
        }
    } catch (err) {
...
async function handleDownload() {
    ...
    try {
        ...
        // Testing / no-payment mode
        if (typeof PAYMENTS_ENABLED !== 'undefined' && !PAYMENTS_ENABLED) {
            let pdfForEmail = null;
            if (product !== 'cover') {
            ...
            if (product !== 'cv') {
                await downloadCoverLetterDocx(data, { fileName: `Cover_Letter_${safeName}.docx` });
            }

            // Send email with CV attached
            if (pdfForEmail) {
                sendCvEmail(data, pdfForEmail, product).catch(() => {});
            }

            saveCvSnapshot().catch(() => {});
            showToast(`${label} downloaded (testing mode).`, 'success');
            if (loadingSpan) loadingSpan.style.display = 'none';
            if (textSpan) textSpan.style.display = 'inline';
            scheduleEntitlementUiRefresh();
            return;
        ...
        // Wait for Lenco if still loading
        if (!window.LencoPay?.getPaid) {
        ...
        window.LencoPay.getPaid({
            ...
            onSuccess: function() {
                ...
                (async () => {
                    try {
                        let pdfForEmail = null;
                        if (product !== 'cover') {
                            // Wait for jsPDF if still loading (started before payment)
                            await jsPdfPromise;
                            const pdf = generatePDF(data);
                            pdfForEmail = pdf;
                            pdf.save(`CV_${safeName}.pdf`);
            ...
            onConfirmationPending: function() {
                showToast('Payment pending. Downloading your CV now...', 'info');
                
                // Wait for jsPDF if still loading, then generate PDF
                (async () => {
                    try {
                        if (product !== 'cover') {
                            await jsPdfPromise;
...
document.addEventListener('DOMContentLoaded', function() {
    // Scripts (Lenco, jsPDF) load from HTML <script> tags
    ...
    // Initialize optional sections UI (keep failures from blocking wizard init)
    const safeCall = (fn) => {
        try { fn(); } catch (e) { console.error('Init failed:', e); }
    };
    safeCall(renderActivitiesHonors);
    safeCall(renderOtherProfiles);
    safeCall(renderCertifications);
    safeCall(renderLanguages);
    safeCall(renderHobbies);
    safeCall(renderReferences);
    ...
    try {
        ...
        // Absolute fallback: show the first step so the user is never stuck with a blank form.
        const first = document.querySelector('.form-step');
